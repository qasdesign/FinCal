# -*- coding: utf-8 -*-
"""
Edited on Thu Sep 23 02:17:00 2021

@editor: QasDesign inspired by Teo Bee Guan
"""
...
with colAnnualSal:
    salary = st.number_input("Enter your annual salary(SR): ", min_value=0.0, format='%f')
with colTax:
    tax_rate = st.number_input("Enter your VAT rate(%): ", min_value=0.0, format='%f')
...
with colExpenses1:
    ...
    monthly_rental = st.number_input("Enter your monthly rental(SR): ", min_value=0.0, format='%f')

    st.subheader("Monthly Food Budget")
    monthly_food = st.number_input("Enter your Monthly food budget (SR): ", min_value=0.0, format='%f')
    # monthly_food = daily_food * 30

    st.subheader("Monthly Unforeseen Expenses")
    monthly_unforeseen = st.number_input("Enter your monthly unforeseen expenses (SR): ", min_value=0.0, format='%f')

with colExpenses2:
    st.subheader("Monthly Transport")
    monthly_transport = st.number_input("Enter your monthly transport fee (SR): ", min_value=0.0, format='%f')

    st.subheader("Monthly Utilities Fees")
    monthly_utilities = st.number_input("Enter your monthly utilities fees (SR): ", min_value=0.0, format='%f')

    st.subheader("Monthly Entertainment Budget")
    monthly_entertainment = st.number_input("Enter your monthly entertainment budget (SR): ", min_value=0.0,
                                            format='%f')
...
st.header("**Savings**")
st.subheader("Monthly Take Home Salary: " + str(round(monthly_takehome_salary, 2))+" SR")
st.subheader("Monthly Expenses: " + str(round(monthly_expenses, 2))+" SR")
st.subheader("Monthly Savings: " + str(round(monthly_savings, 2))+" SR")
